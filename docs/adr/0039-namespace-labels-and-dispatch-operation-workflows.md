...
PullOps uses namespaced Operation Labels with the `pullops:<target-kind>:<operation>` grammar. A dispatcher workflow listens for GitHub label events and triggers the separate operation workflows through `workflow_dispatch`, preserving per-operation workflow readability and permissions. Merged Child Issue PR closure is not label-dispatched; it is GitHub State Synchronization handled directly from `pull_request.closed`.
...
Dependency checks use issue closure, so child issue dependencies are satisfied only after the blocking Child Issue PR has merged and PullOps has closed the child issue.
