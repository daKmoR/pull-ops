...
Operation Labels are derived from dispatchable catalog entries. PullOps Status Labels, including `pullops:human-required`, stay separate because they are not Operation identity. Workflow-facing operations that have no Operation Label Reference, such as `pr-close-child-issue`, are still catalog entries with no dispatchable label reference.
