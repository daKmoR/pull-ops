...
test('setup doctor returns zero for changes-needed setup guidance', async () => {
  ...
  const stdout = createWritableBuffer();
  const cli = new PullOpsCli({ cwd, stdout });
  const exitCode = await cli.run(['setup', 'doctor', '--profile', 'full', '--json']);

  assert.equal(exitCode, 0);

  const output = JSON.parse(stdout.text);
  assert.equal(output.status, 'changes-needed');
  assert.equal(output.area, 'setup-doctor');
  assert.ok(output.changesNeeded.includes('.agents/skills/pullops-pr-review/SKILL.md'));
