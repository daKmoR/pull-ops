...
import { readPullOpsPullRequestState } from '../review-pr/prBody.js';
import { validatePrepareMergeOutput } from './output.js';
import { buildPrepareMergePrompt } from './prompt.js';
import {
  updatePullRequestBodyForPrepareMerge,
  updatePullRequestBodyForPrepareMergeFailure,
} from './prBody.js';
 ...
 */
export async function runPrepareMerge(context) {
  const preparation = await prepareMerge(context);
  ...
  try {
    rawOutput = await context.codexRunner.run({
      ...
      model: context.model,
      prompt: buildPrepareMergePrompt({
  ...
  return await finalizePreparedMerge(context, preparation, rawOutput);
}
 ...
 * @returns {Promise<Record<string, unknown>>}
 */
export async function runPrepareMergeCodexActionPrepare(context) {
  const preparation = await prepareMerge(context);
  if (!preparation.ready) {
    return preparation.output;
  }
  ...
  return {
    status: 'accepted',
    summary: `Prepared Codex Action prepare-merge run for PR #${preparation.pullRequest.number}.`,
    pullRequest: {
 ...
 * @returns {Promise<Record<string, unknown>>}
 */
export async function runPrepareMergeCodexActionFinalize(context) {
...
/**
 * @param {OperationRunnerContext} context
 * @returns {Promise<PrepareMergePreparation>}
 */
async function prepareMerge(context) {
...
/**
 * @param {OperationRunnerContext} context
 * @param {PrepareMergePreparation & { ready: true }} preparation
 * @param {unknown} rawOutput
 * @returns {Promise<Record<string, unknown>>}
 */
async function finalizePreparedMerge(context, preparation, rawOutput) {
  const { pullRequest, changedFiles, sourceKind, sourceIssueNumber, baseBranch } = preparation;
  let failureRecorded = false;

  try {
    const validatedOutput = validatePrepareMergeOutput(rawOutput);
    ...
    const commits = validatedOutput.value.commitPlan.commits.map(commit => ({
      message: createPrepareMergeCommitMessage(commit),
      files: commit.files,
    }));
    ...
    await context.githubClient.updatePullRequestBody({
      number: pullRequest.number,
      body: updatePullRequestBodyForPrepareMerge({
 ...
 * @returns {string}
 */
export function createPrepareMergeCommitMessage(commit) {
  return [
    commit.header,
 ...
 * @param {PlannedPrepareMergeOutput} options.output
...
async function transitionPullRequestLabelsToReview(context, pullRequest) {
  await context.githubClient.removeLabelsFromPullRequest({
    number: pullRequest.number,
    labels: [PULL_OPS_OPERATION_LABELS.prepareMerge, ...PULL_OPS_STATUS_LABEL_NAMES],
  });
...
async function recordPullRequestFailure(context, pullRequest, reason, { updateBody = true } = {}) {
  ...
  if (updateBody) {
    await context.githubClient.updatePullRequestBody({
      number: pullRequest.number,
      body: updatePullRequestBodyForPrepareMergeFailure({
        body: pullRequest.body,
      }),
    });
  }
  ...
  await context.githubClient.commentOnPullRequest({
    number: pullRequest.number,
    body: ['PullOps could not complete `pullops run prepare-merge`.', '', `Reason: ${reason}`].join(
      '\n',
    ),
  });
}
...
function assertPullRequestTarget(context) {
  if (context.target.type !== 'pr') {
    throw new Error('prepare-merge requires a pull request target.');
  }
}
