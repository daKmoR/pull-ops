...
import {
  ...
  coordinateLocalSpecAutoAdvance,
  coordinateLocalSpecAutoComplete,
  resumeSpecAutomationForParentIssue as resumeSpecAutomation,
 ...
 */
export async function runSpecAutoAdvance(context) {
  assertIssueTarget(context, 'spec-auto-advance');
  if (context.executionBackend === 'local') {
    const localContext = withDefaultLocalSpecRunGoal(context);
    return await coordinateLocalSpecAutoAdvance(localContext, {
      parentIssueNumber: localContext.target.number,
      async runTicket(ticketNumber, options = {}) {
  ...
  return await coordinateSpecAutomation(context, {
 ...
 * @returns {Promise<Record<string, unknown>>}
 */
export async function runSpecAutoComplete(context) {
  assertIssueTarget(context, 'spec-auto-complete');
  if (context.executionBackend === 'local') {
    const localContext = withDefaultLocalSpecRunGoal(context);
    return await coordinateLocalSpecAutoComplete(localContext, {
      parentIssueNumber: localContext.target.number,
      async runTicket(ticketNumber, options = {}) {
        if (shouldRunNestedExternalHandoffs(localContext)) {
          return await runLocalPublishedExternalIssueOperation(localContext, {
      ...
      async runTicketPullRequestOperation({ pullRequestNumber, operation, parentRun }) {
        return await runLocalPublishedPullRequestOperation(localContext, {
          pullRequestNumber,
          operation,
          resumeParentSpecAutomationAfterPrFinalize: false,
          ...(parentRun === undefined ? {} : { parentRun }),
        });
      },
    });
 ...
 * @param {{
 *   ticketNumber: number,
 *   options: import('../../spec-automation/ticketCoordination.types.js').TicketRunOptions,
 * }} request
 * @returns {Promise<Record<string, unknown>>}
 */
async function runLocalPublishedExternalIssueOperation(context, { ticketNumber, options }) {
...
/**
 * Resume whichever Spec automation mode is active on a Parent Issue.
 *
 * @param {OperationRunnerContext} context
...
export async function resumeSpecAutomationForParentIssue(context, parentIssueNumber) {
  return await resumeSpecAutomation(context, parentIssueNumber);
}
...
/**
 * Local Spec runs should prepare the same finalized ticket PR branch that publication will push,
 * while callers can still request operation-only behavior explicitly.
 *
 * @param {OperationRunnerContext} context
 * @returns {OperationRunnerContext}
 */
function withDefaultLocalSpecRunGoal(context) {
...
async function runLocalPublishedPullRequestOperation(
  context,
  { pullRequestNumber, operation, resumeParentSpecAutomationAfterPrFinalize, parentRun },
) {
  ...
  const operationContext = createPullRequestOperationContext(context, {
    pullRequestNumber,
    operation,
    resumeParentSpecAutomationAfterPrFinalize,
    parentRun,
...
function createPullRequestOperationContext(
  ...
  return {
    ...
    suppressFollowUpOperationLabels: true,
    ...(resumeParentSpecAutomationAfterPrFinalize === undefined
      ? {}
      : { resumeParentSpecAutomationAfterPrFinalize }),
  };
}
