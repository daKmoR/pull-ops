 ...
 * @param {PrAddressReviewFeedbackItem[]} options.feedbackItems
 * @returns {string}
 */
export function buildAddressPrReviewompt({
  pullRequest,
  issue,
  reviewContext,
  diff,
  ...
  return [
    'Use the pullops-pr-address-review skill.',
    '',
    ...
    '',
    'Boundaries:',
    '- Classify each supplied feedbackId as addressed, declined, or deferred: address by default; decline only when the requested change should not be made, with a substantive reason; defer only when the feedback is stale, irrelevant, or outside this PR, with a reason.',
