...
import { createSpecPrepareCommitMessage, runSpecPrepare } from './run.js';
...
describe('runSpecPrepare', () => {
  it('01: creates an umbrella branch and draft PR for a parent issue', async () => {
    ...
    const result = await runSpecPrepare(
      createContext({
    ...
    assert.deepEqual(git.pushes, [{ branchName: 'pullops/spec-12' }]);
    assert.equal(github.createdPullRequests.length, 1);
    assert.equal(github.createdPullRequests[0].title, 'Prepare #12: Spec: Parent workflow');
    assert.equal(github.createdPullRequests[0].baseBranch, 'main');
    assert.equal(github.createdPullRequests[0].headBranch, 'pullops/spec-12');
    ...
    assert.match(
      ...
      /<summary>PullOps workflow state<\/summary>[\s\S]*Last operation: pullops:spec:prepare/,
    );
    ...
    assert.match(github.createdPullRequests[0].body, /^Spec Issue: #12$/m);
    ...
    assert.doesNotMatch(github.createdPullRequests[0].body, /pullops\/spec-12/);
    assert.doesNotMatch(github.createdPullRequests[0].body, /#34 Implement ticket workflow/);
    ...
    assert.deepEqual(github.issueLabelsRemoved, [
      ...
      {
        number: 12,
        labels: ['pullops:spec:prepare', 'pullops:human-required'],
      },
    ]);
  });

  it('02: updates an existing umbrella PR with known Ticket PR links', async () => {
    const issue = createIssue({
      number: 12,
      subIssues: [
        {
    ...
    const github = createFakeGitHub({
      ...
      openPullRequestsByHead: new Map([['pullops/spec-12-issue-34', ticketPullRequest]]),
    });
    const git = createFakeGit();
    ...
    assert.match(github.updatedPullRequestBodies[0].body, /Last operation: pullops:spec:prepare/);
    assert.deepEqual(github.issueLabelsAdded, []);
    assert.deepEqual(github.issueLabelsRemoved, [
      {
        number: 12,
  ...
  it('03: blocks prepare when applied to a known ticket', async () => {
    const issue = createIssue({
      number: 34,
      parent: {
        number: 12,
    ...
    assert.equal(git.branches.length, 0);
    assert.match(github.comments[0].body, /pullops:spec:prepare on the parent issue/);
  });
});
