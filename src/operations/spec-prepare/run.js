...
import { createSpecPreparePullRequestBody } from './prBody.js';
 ...
 */
export async function runSpecPrepare(context) {
  ...
  if (parentIssueNumber !== undefined) {
    return await blockPreparation(context, issue, {
      reason: [
        ...
        [
          `Use ${requireOperationCatalogOperationLabelName('issue-implement')} on concrete tickets,`,
          `or ${requireOperationCatalogOperationLabelName('spec-prepare')} on the parent issue.`,
        ].join(' '),
      ].join(' '),
    });
  }
  ...
  const pullRequestBody = await createSpecPreparePullRequestBodyForIssue(context, {
    issue,
    branchName,
  });
  ...
  try {
    ...
    await context.gitClient.commitEmpty({
      message: createSpecPrepareCommitMessage(issue),
 ...
 * @returns {string}
 */
export function createSpecPrepareCommitMessage(issue) {
  return [
    `chore(spec): prepare #${issue.number}`,
    '',
 ...
 */
export async function createSpecPreparePullRequestBodyForIssue(context, { issue, branchName }) {
  const ticketPullRequests = await readTicketPullRequests(context, { issue });
  return createSpecPreparePullRequestBody({
    issue,
    ticketPullRequests,
    branchName,
...
function assertIssueTarget(context) {
  if (context.target.type !== 'issue') {
    throw new Error('spec-prepare requires an issue target.');
  }
}
...
async function blockPreparation(context, issue, { reason }) {
  ...
  await context.githubClient.removeLabelsFromIssue({
    number: issue.number,
    labels: [requireOperationCatalogOperationLabelName('spec-prepare')],
  });
  await context.githubClient.commentOnIssue({
    number: issue.number,
    body: ['PullOps could not complete `pullops run spec-prepare`.', '', `Reason: ${reason}`].join(
      '\n',
    ),
  });
...
async function markPreparationPrepared(context, issue) {
  await context.githubClient.removeLabelsFromIssue({
    number: issue.number,
    labels: [
      requireOperationCatalogOperationLabelName('spec-prepare'),
 ...
 */
async function readTicketPullRequests(context, { issue }) {
  /** @type {{ issueNumber: number, pullRequest: GitHubPullRequest }[]} */
  const ticketPullRequests = [];

  for (const ticket of issue.subIssues) {
    if (ticket.relationshipSource !== 'native') {
      continue;
    }
    ...
    const pullRequest = await findPullRequestByHead(context, ticketBranchName);
    if (pullRequest !== undefined) {
      ticketPullRequests.push({
        issueNumber: ticket.number,
        pullRequest,
      });
    }
