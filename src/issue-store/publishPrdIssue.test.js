...
describe('publishPrdIssue', () => {
  it('01: creates a PullOps-published PRD issue with a triage role and run record artifacts', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'pullops-publish-prd-create-'));
    ...
    const result = await publishPrdIssue({
      cwd,
      config: {
    ...
    assert.deepEqual(result, {
      status: 'accepted',
      summary: 'Created PullOps-published PRD Issue #17.',
      action: 'created',
      issue: {
        number: 17,
      ...
      warnings: [],
      localRunRecord: join(cwd, '.pullops', 'runs', '2026-06-20T101500000Z-issues-publish-prd-new'),
      triageRole: 'ready-for-agent',
    });
  ...
  it('02: updates only PullOps-published issues and removes conflicting triage labels', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'pullops-publish-prd-update-'));
    const github = createFakeGitHubClient({
      /** @param {number} number */
    ...
    assert.deepEqual(result, {
      status: 'accepted',
      summary: 'Updated PullOps-published PRD Issue #41.',
      action: 'updated',
      issue: {
        number: 41,
      ...
      warnings: [],
      localRunRecord: join(cwd, '.pullops', 'runs', '2026-06-20T101500000Z-issues-publish-prd-41'),
      triageRole: 'ready-for-human',
    });
  ...
  it('03: rejects malformed JSON input with a stable failure output and run record artifacts', async () => {
    ...
    assert.equal(result.summary, 'Publish PRD request failed.');
    assert.match(result.failureReason, /Publish request must be valid JSON:/);
    assert.equal(
      result.localRunRecord,
      join(cwd, '.pullops', 'runs', '2026-06-20T101500000Z-issues-publish-prd-invalid'),
    );
    assert.equal(
  ...
  it('04: rejects conflicting PRD fields with a stable failure output and run record artifacts', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'pullops-publish-prd-conflict-'));
    const github = createFakeGitHubClient();
  ...
  it('05: refuses to update unmarked manual issues', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'pullops-publish-prd-unmarked-'));
    const github = createFakeGitHubClient({
      /** @param {number} number */
      async getIssue(number) {
    ...
    const result = await publishPrdIssue({
      ...
      rawRequest: {
        ...
        solution: 'Do the thing in PRD form.',
        userStories: [
          {
            number: 1,
            story:
    ...
    assert.equal(result.status, 'failed');
    assert.equal(result.summary, 'Refused to update PRD issue #88.');
