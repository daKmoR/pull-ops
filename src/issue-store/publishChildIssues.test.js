...
describe('publishChildIssues', () => {
  ...
  it('07: reuses existing marker-owned children on plain rerun and continues dependency resolution', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'pullops-publish-children-rerun-reuse-'));
        ...
        if (number === 126) {
          return createIssue({
            ...
            body: createPrdIssueBody({
              ...
              problemStatement: 'Parent problem.',
              solution: 'Parent solution.',
              userStories: [{ number: 23, story: 'As an agent, I can reuse rerun children.' }],
              implementationDecisions: ['Reuse marker-owned children on rerun.'],
              testingDecisions: ['Use focused tests.'],
              outOfScope: ['Unrelated publication.'],
              furtherNotes: [],
              auditDetails: [],
    ...
    assert.deepEqual(github.subIssueAdds, [{ parentIssueNumber: 126, childIssueNumber: 202 }]);
  });

  it('08: plain reruns recover unattached marker-owned children from prior run records', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'pullops-publish-children-rerun-recover-'));
    const request = {
      parentIssueNumber: 126,
      children: [
        {
          ...
          acceptanceCriteria: ['The existing child is reused instead of duplicated.'],
          coveredUserStories: [23],
          triageRole: 'ready-for-agent',
        },
      ],
    };
    const recoveredChild = createIssue({
      number: 201,
      title: 'Recover child',
      body: createChildIssueBody({
        parentIssueNumber: 126,
      ...
      async getIssue(number) {
        ...
        return createIssue({
          ...
          body: createPrdIssueBody({
            ...
            implementationDecisions: ['Recover unattached children from prior run records.'],
    ...
    const firstResult = await publishChildIssues({
      cwd,
      config: { issueStore: { provider: 'github' } },
    ...
    assert.deepEqual(firstResult.children, [
      {
        sliceRef: 'repair',
        action: 'created',
        issue: {
    ...
    assert.deepEqual(firstResult.failedChildren, [
      {
        sliceRef: 'repair',
    ...
    const secondGithub = createFakeGitHubClient({
      ...
      async removeLabelsFromIssue(options) {
        ...
        recoveredChild.labels = recoveredChild.labels.filter(
      ...
      async addLabelsToIssue(options) {
        ...
        recoveredChild.labels = [...new Set([...recoveredChild.labels, ...options.labels])];
      },
    });

    const secondResult = await publishChildIssues({
      cwd,
      config: { issueStore: { provider: 'github' } },
    ...
    assert.deepEqual(secondGithub.subIssueAdds, [
      { parentIssueNumber: 126, childIssueNumber: 201 },
    ]);
    ...
    assert.deepEqual(secondResult.children, [
      {
        sliceRef: 'repair',
        action: 'updated',
        issue: {
  ...
  it('09: force-updates existing marker-owned children by parent and slice ref on rerun', async () => {
  ...
  it('11: explicit issue number overrides repair unattached marker-owned children safely', async () => {
  ...
  it('12: explicit issue number overrides ignore stale recovery records for the same slice ref', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'pullops-publish-children-override-stale-recovery-'));
    await writeRecoveredChildIssueRunRecord({
      cwd,
      runId: '2026-06-20T101600000Z-issues-publish-children-126',
      parentIssueNumber: 126,
      mappings: [{ sliceRef: 'repair', issueNumber: 202 }],
    });
    await writeRecoveredChildIssueRunRecord({
      cwd,
      runId: '2026-06-20T101500000Z-issues-publish-children-126',
      parentIssueNumber: 126,
    ...
    assert.deepEqual(github.subIssueAdds, [{ parentIssueNumber: 126, childIssueNumber: 201 }]);
    assert.equal(result.children[0].issue.number, 201);
  });

  it('13: plain reruns prefer the newest valid recovery record for a slice ref', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'pullops-publish-children-recovery-newest-valid-'));
    await writeRecoveredChildIssueRunRecord({
      cwd,
      runId: '2026-06-20T101600000Z-issues-publish-children-126',
      parentIssueNumber: 126,
  ...
  it('14: ignores unrelated corrupt local run records during recovery', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'pullops-publish-children-recovery-filter-'));
    await writeRecoveredChildIssueRunRecord({
      cwd,
      runId: '2026-06-20T101500000Z-issues-publish-children-126',
      parentIssueNumber: 126,
 ...
 */
async function writeRecoveredChildIssueRunRecord({ cwd, runId, parentIssueNumber, mappings }) {
