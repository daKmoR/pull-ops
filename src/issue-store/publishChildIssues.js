    ...
    try {
      ...
      failedChildren.push({
        ...
        ...(publishedChild === undefined
          ? {}
          : {
              ...(publishedChild.action === 'created' || publishedChild.action === 'updated'
 ...
 */
async function readExistingPublishedChildrenBySliceRef({
  cwd,
  githubClient,
  parentIssue,
  ...
  const recoveredChildIssueRefs = await readRecoveredChildIssueRefsByNewestRun({
    cwd,
    parentIssueNumber: parentIssue.number,
  });
  for (const [sliceRef, issueNumber] of recoveredChildIssueRefs) {
    if (explicitOverrideSliceRefs.has(sliceRef) || existingChildrenBySliceRef.has(sliceRef)) {
      continue;
    }
    ...
    existingChildrenBySliceRef.set(sliceRef, issue);
  }
...
/**
 * @param {NormalizedChildIssueBatchRequest} normalizedRequest
 * @returns {Set<string>}
 */
function readExplicitIssueNumberOverrideSliceRefs(normalizedRequest) {
  return new Set(
    normalizedRequest.children
      .filter(childRequest => childRequest.issueNumber !== undefined)
      .map(childRequest => childRequest.sliceRef),
  );
}
 ...
 */
function selectExistingChild({ childRequest, existingChildrenBySliceRef }) {
  if (childRequest.issueNumber !== undefined) {
    return undefined;
  }

  return existingChildrenBySliceRef.get(childRequest.sliceRef);
}
...
async function publishOneChildIssue({
  ...
  if (existingChild !== undefined) {
    if (!forceUpdate) {
      if (
        shouldRepairRecoveredChildIssue({
          childRequest,
          existingIssue: existingChild,
          parentIssueNumber,
        })
      ) {
        return await repairRecoveredChildIssue({
          githubClient,
          childRequest,
          existingIssue: existingChild,
          parentIssueNumber,
 ...
 * @returns {boolean}
 */
function shouldRepairRecoveredChildIssue({ childRequest, existingIssue, parentIssueNumber }) {
 ...
 */
function reuseChildIssue({ childRequest, existingIssue, resolvedBlockedBy }) {
  return createPublishedChild(childRequest, existingIssue, 'reused', resolvedBlockedBy);
}
 ...
 * @returns {Promise<ChildIssuePublishChild>}
 */
async function repairRecoveredChildIssue({
  githubClient,
  childRequest,
  existingIssue,
  parentIssueNumber,
  ...
  let publishedChild;
  try {
    publishedChild = createPublishedChild(
      childRequest,
      existingIssue,
      'updated',
      resolvedBlockedBy,
 ...
 */
async function readRecoveredChildIssueRefsByNewestRun({ cwd, parentIssueNumber }) {
  ...
  const recoveredChildIssueRefs = [];
  const recordNames = entries
    .filter(
      entry =>
        entry.isDirectory() && isChildIssuePublicationRunRecordName(entry.name, parentIssueNumber),
    )
  ...
  for (const recordName of recordNames) {
    ...
    for (const [sliceRef, issueNumber] of readRecoveredChildIssueRefs(response)) {
      recoveredChildIssueRefs.push([sliceRef, issueNumber]);
    }
  }

  return recoveredChildIssueRefs;
}
 ...
 * @returns {boolean}
 */
function isChildIssuePublicationRunRecordName(recordName, parentIssueNumber) {
  return recordName.endsWith(`-issues-publish-children-${parentIssueNumber}`);
}
 ...
 */
function readRecoveredChildIssueRefs(response) {
  ...
  for (const mapping of mappings) {
    ...
    childIssueRefs.push([sliceRef, issueNumber]);
  }

  return childIssueRefs;
}
