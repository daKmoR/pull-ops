...
2. Run `pullops setup doctor --profile full --json` first and read the structured blockers and warnings. Completion criterion: every blocker and warning is classified as local action, remote approval, or external wait.
...
- Skills: use `pullops setup skills --check --json`, then `pullops setup skills --json`.
- Agent docs: use `pullops setup agent-docs --check --json`, then `pullops setup agent-docs --json`.
- GitHub Actions: use `pullops setup github-actions --check --json`, then `pullops setup github-actions --json`.
- Re-run `pullops setup doctor --profile github-actions --json` after workflow setup to confirm GitHub Actions readiness.
- GitHub labels: use `pullops setup github-labels --check --json` to inspect PullOps labels. Ask before running `pullops setup github-labels --json` because it mutates the remote repository.
